...
import { Icon, BitintLogo } from './ui/Icons';

export function AppChrome({ title='app.bitint.io', children, style, height = 560 }: { title?: string, children: React.ReactNode, style?: React.CSSProperties, height?: number }) {
  return (
    <div style={{
      borderRadius: 16,
  ...
  return (
    <div style={{
      ...
      <div style={{padding:'4px 8px 12px', display:'flex', alignItems:'center', gap:8}}>
        <BitintLogo size={22} showWordmark={true} />
      </div>
      {items.map(it => (
        <div key={it.k} style={{
          display:'flex', alignItems:'center', gap:10,
...
export function ProductTopbar({ searchValue='1A1zP1eP5QGefi2DMPTfTL5SLmv7DivfNa' }: { searchValue?: string }) {
  return (
    <div style={{
...
export function InvestigationGraph({ animated = true }: { animated?: boolean }) {
  const nodes = useMemo(() => ([
...
export function MonitoringMini() {
  const rows = [
  ...
  const riskColor = (r: number) => r >= 90 ? '#ef4444' : r >= 70 ? '#f59e0b' : '#10b981';
  return (
    <div style={{display:'flex', height:'100%'}}>
...
export function WalletScreeningMini() {
  return (
    <div style={{display:'flex', height:'100%'}}>
        ...
        <div style={{padding:18, overflow:'hidden'}}>
          ...
          <div style={{display:'grid', gridTemplateColumns:'1fr 1fr 1fr', gap:12}}>
            {[
              ...
              const toneBg: any = {emerald:'rgba(16,185,129,0.06)', red:'rgba(239,68,68,0.06)', amber:'rgba(245,158,11,0.08)'};
              const toneFg: any = {emerald:'#059669', red:'#dc2626', amber:'#d97706'};
              return (
...
export function CaseManagerMini() {
  const cases = [
